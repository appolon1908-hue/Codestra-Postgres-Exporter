...
search_root="${1:-.}"
pattern="(BEGIN ([A-Z0-9][A-Z0-9 -]{0,63} )?PRIVATE KEY|[\"']?Authorization[\"']?[[:space:]]*:[[:space:]]*[\"']?[[:space:]]*Bearer[[:space:]]+[-A-Za-z0-9._~+/]{16,}=*|[\"']?[A-Za-z0-9_-]*client[_-]?secret[\"']?[[:space:]]*[:=][[:space:]]*[^[:space:]<]+|postgres(ql)?://[^[:space:]/:@]+:[^[:space:]@/]+@|[\"']?(PGPASSWORD|POSTGRES(QL)?[_-]?PASSWORD|DATABASE[_-]?PASSWORD|DB[_-]?PASSWORD)[\"']?[[:space:]]*[:=][[:space:]]*[\"']?[^[:space:]<\"']{4,})"
path_list="$(mktemp)"
